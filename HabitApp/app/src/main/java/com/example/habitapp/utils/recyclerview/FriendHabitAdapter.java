 ...
 * @version 0.1
 */
public class FriendHabitAdapter extends RecyclerView.Adapter<HabitViewHolder> {
    ArrayList<Habit> habitsToDisplay;
    private OnHabitListener mOnHabitListener;
    String tag;
    FirebaseFirestore fRef;

    public FriendHabitAdapter(ArrayList<Habit> habitsToDisplay, OnHabitListener OnHabitListener) {
        this.habitsToDisplay = habitsToDisplay;
        this.mOnHabitListener = OnHabitListener;
    }

    public FriendHabitAdapter(ArrayList<Habit> habitsToDisplay, OnHabitListener OnHabitListener, String tag) {
        this.habitsToDisplay = habitsToDisplay;
        this.mOnHabitListener = OnHabitListener;
        this.tag = tag;
    }
    ...
    public void onBindViewHolder(@NonNull final HabitViewHolder holder, final int position) {
        ...
        fRef.collection(HabitConstants.USER_PATH).document(habitsToDisplay.get(position).getOwnerID()).get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful()) {
                    DocumentSnapshot document = task.getResult();
                    setText(holder, position, document.toObject(User.class).getDisplayName().split(" ")[0]);
                } else {
                    setText(holder, position, "A friend");
                }
            }
        });
    ...
    private void setText(HabitViewHolder holder, int position, String name) {
        holder.titleText.setText(name + " has updated " + habitsToDisplay.get(position).getTitle());
